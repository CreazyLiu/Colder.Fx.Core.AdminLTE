...
namespace Aliyun.Acs.Core.Auth
{
    public class ShaHmac1 : ISigner
    {
        ...
        public override String SignString(String source, String accessSecret)
        {
            ...
            using (var algorithm = ShaHmacHelper.CreateAlgorithm("HMACSHA1", Encoding.UTF8.GetBytes(accessSecret.ToCharArray())))
            {
                return Convert.ToBase64String(algorithm.ComputeHash(Encoding.UTF8.GetBytes(source.ToCharArray())));
            }
        }
    ...
    internal static class ShaHmacHelper
    {
        internal static HMAC CreateAlgorithm(string method, byte[] key)
        {
