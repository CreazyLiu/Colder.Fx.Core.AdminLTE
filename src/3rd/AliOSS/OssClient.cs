...
namespace Aliyun.OSS
{
    ...
    public class OssClient : IOss
    {
        ...
        {
            if (endpoint == null)
                throw new ArgumentException("ExceptionIfArgumentStringIsNullOrEmpty"/*Resources.ExceptionIfArgumentStringIsNullOrEmpty*/, "endpoint");
            ...
            if (credsProvider == null)
                throw new ArgumentException("ExceptionIfArgumentStringIsNullOrEmpty"/*Resources.ExceptionIfArgumentStringIsNullOrEmpty*/, "credsProvider");
        ...
        public bool DoesBucketExist(string bucketName)
        {
            if (string.IsNullOrEmpty(bucketName))
                throw new ArgumentException("ExceptionIfArgumentStringIsNullOrEmpty"/*Resources.ExceptionIfArgumentStringIsNullOrEmpty*/, "bucketName");
        ...
        public string CalculatePostSignature(string postPolicy)
        {
            try
            {
                ...
                var encPolicy = Convert.ToBase64String(binaryData);
                return ServiceSignature.Create().ComputeSignature(_credsProvider.GetCredentials().AccessKeySecret, encPolicy);
            }
            catch (Exception ex)
            {
                throw new ClientException("Unsupported charset: " + ex.Message);
            }
        }
        ...
        #region Sign
        public string HmacSha1Sign(string source)
        {
            try
            {
                return ServiceSignature.Create().ComputeSignature(_credsProvider.GetCredentials().AccessKeySecret, source);
            }
            catch (Exception ex)
            {
                throw new ClientException("HmacSha1Sign fail: " + ex.Message);
            }
        }
        #endregion
